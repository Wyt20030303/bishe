...
import com.pan.email.util.RedisKeyUtil;
...
public class LoginController implements CommunityConstant {
    ...
    @ResponseBody
    public Map forget(String email, String userName, String password) {
        Map map = new HashMap();
        User user = userService.findUserByEmailAndName(email, userName);
        if(user == null){
            map.put("code", 1);
            map.put("msg", "填写信息有误，请重新填写并提交");
            return map;
        }
    ...
    public String register(Model model, User user) {
        ...
        if (map == null || map.isEmpty()) {
            model.addAttribute("msg", "注册成功, 您已自动激活，即将进入邮箱主页哦!");
     ...
     * @param code 激活码
     * @return
     * http://localhost:8080/email/activation/用户id/激活码
     */
     ...
     */
    @GetMapping("/kaptcha")
    public void getKaptcha(HttpServletRequest re, HttpServletResponse response) {
        // 生成验证码
    ...
    public String login(@RequestParam("username") String username,
                        ...
                        HttpServletRequest re, @CookieValue("kaptchaOwner") String kaptchaOwner) {
         //检查验证码
        String kaptcha = null;
